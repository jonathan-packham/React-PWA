...
const Identifyhazards = ({prevStep, SaveAndExit, nextStep, handleChange, values}) => {
  const Continue = e => {
    e.preventDefault();
    nextStep();
  }
      ...
      <form className='Hazard-form'>
        <div className='Hazard-form-content'>
          ...
          <div style={{height: 200}} className='mt-3 overflow-auto'>
            <label className='form-label' for='idenHaz'>Identify Hazards associated with each step</label>
            <textarea className='form-control' id='idenHaz' rows='4' value={values.idenHaz} onInput={handleChange('idenHaz')}></textarea>
          </div>
