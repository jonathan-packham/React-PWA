...
const Jobsteps = ({prevStep, SaveAndExit, nextStep, handleChange, values}) => {
  const Continue = e => {
    e.preventDefault();
    nextStep();
  }
    ...
    <div className='Hazard-form-container'>
      <form className='Hazard-form'>
        <div className='Hazard-form-content'>
          ...
          <h5 className='Hazard-form-subtitle'>Sequence of Job Steps</h5>
          <div style={{height: 200}} className='mt-3 overflow-auto'>
            <label className='form-label' for='jobSteps'>Identify the required steps to complete the job</label>
            <textarea className='form-control' id='jobSteps' rows='4' value={values.jobSteps} onInput={handleChange('jobSteps')}></textarea>
          </div>
